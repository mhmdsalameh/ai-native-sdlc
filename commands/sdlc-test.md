...
3. For bug fixes: reproduce the bug as a failing test, confirm it fails **for the expected reason**, then fix the code — not the test. The guard blocks editing tests during a fix. For UI, use a browser/screenshot tool and iterate 2–3 rounds against the mock.
